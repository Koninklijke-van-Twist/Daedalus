 ...
 * Mímir beheert environments — Daedalus heeft alleen de API-key (+ optioneel $mimirBase) nodig.
 *
...
function odata_mimir_request(string $method, string $path, ?array $jsonBody = null): array
{
    ...
    $opts = [
        ...
        CURLOPT_USERAGENT => 'Daedalus-MimirClient/1.0',
    ];
    if ($jsonBody !== null) {
...
function odata_get_all(string $url, array $auth, $ttlSeconds = 300): array
{
    ...
    if (odata_mimir_enabled()) {
        ...
        return odata_normalize_text_newlines_recursive($rows);
    }
...
{
    // Local file cache is only used when Mímir is off; still load auth for BC cache keys.
    if (!odata_mimir_enabled()) {
